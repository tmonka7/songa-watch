...
Smart watch firmware for the **Waveshare ESP32-S3-Touch-AMOLED-2.06** — a
410×502 AMOLED, capacitive touch, 6-axis IMU, PMU, RTC, microphones, speaker
...
| `esp_lvgl_port` | 2.9.0 | LVGL ↔ esp_lcd glue |
| `esp_codec_dev` | 1.5.11 | ES8311 speaker, ES7210 microphone |
| `esp_lcd_panel_io_additions`, `esp_io_expander`, `cmake_utilities` | — | BSP dependencies |

About 75 MB. LVGL's `tests/`, `docs/`, the built-in font sources and the
non-ESP platform ports were removed — none are referenced by
...
Anything that blocks — an ELM327 exchange, a BLE connect — runs on a worker
...
| Dimmed | `idle_dim_sec` (10 s) | brightness ÷ 4 |
...
- **The SoC leaves light sleep on an interrupt, not a timer.** The touch
  ...
  `lvgl_port_stop()` disables LVGL's timers — and therefore its reading of
  the touch device — the power task polls that same line at 100 ms *while
...
- **Bounded reconnects.** Four attempts, then stop — a station stuck in a
...
unavailable: GPIO 0 (BOOT), 19/20 (USB), 26–37 (flash + octal PSRAM), 43/44
(console).

**The PWR button is wired to the AXP2101, not to the SoC.** Presses arrive
as PMU interrupts, never as a GPIO edge — short press toggles the display,
...
| Weather (22 °C, Cloudy) | No weather source without an internet API key | **Ambient temperature** from the QMI8658 die sensor, labelled as temperature |
...
| Camera | No camera and no free DVP bus — a parallel sensor needs 13 pins, this board has 8 left | An **Arducam Mega** (3 MP / 5 MP) on SPI, 4 wires. See below |
...
Preview frames are pulled as **RGB565, not JPEG** — the module can emit
either, and taking RGB565 keeps a JPEG decoder out of the dependency set
entirely. The cost is bandwidth: 320×240×2 over SPI at 8 MHz is about 6 fps.
...
change energy. It runs in real time on the S3 and genuinely detects things —
...
Nordic-UART-style write/notify pair — the ones advertising as `OBDBLE`,
...
English and Japanese, switchable without a reboot — `WATCH_EV_LANG_CHANGED`
...
outside it draws as nothing, and the build gives no warning — so this is
...
Both run without ESP-IDF installed.
